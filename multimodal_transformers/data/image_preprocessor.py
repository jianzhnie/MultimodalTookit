...
class ImagePreprocessor(BasePreprocessor):
    ...
    def transform(self, df: pd.DataFrame) -> np.ndarray:
        ...
        imgs = [
            cv2.imread('/'.join([self.img_path, img])) for img in image_list
        ]
        ...
        resized_imgs = []
        for i, img in tqdm(
                enumerate(imgs), total=len(imgs), disable=self.verbose != 1):
            if i in diff_idx:
        ...
        if self._compute_normalising_metrics:
            ...
            for rsz_img in resized_imgs:
                (mean_b, mean_g, mean_r), (std_b, std_g,
                                           std_r) = cv2.meanStdDev(rsz_img)
