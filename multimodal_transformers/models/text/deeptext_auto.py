...
class AutoModelWithText:
    ...
    def __init__(self):
        raise EnvironmentError(
            'AutoModelWithText is designed to be instantiated '
            'using the `AutoModelWithText.from_pretrained(pretrained_model_name_or_path)` or '
            '`AutoModelWithText.from_config(config)` methods.')
    ...
    def from_config(cls, config):
        ...
        Examples::
            ...
            model = AutoModelWithText.from_config(config)  # E.g. model was saved using `save_pretrained('./test/saved_model/')`
        """
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args,
        ...
        Examples::
            ...
            model = AutoModelWithText.from_pretrained('bert-base-uncased')    # Download model and configuration from S3 and cache.
            model = AutoModelWithText.from_pretrained('./test/bert_model/')  # E.g. model was saved using `save_pretrained('./test/saved_model/')`
            assert model.config.output_attention == True
            # Loading from a TF checkpoint file instead of a PyTorch model (slower)
            config = AutoConfig.from_json_file('./tf_model/bert_tf_model_config.json')
            model = AutoModelWithText.from_pretrained('./tf_model/bert_tf_checkpoint.ckpt.index', from_tf=True, config=config)
